"""Testes de app_recebimento.services.data_writer — deduplicação por row_hash."""
...
def _raw_df(rows: list[dict]) -> pd.DataFrame:
    ...
    for r in rows:
        base.append({
            RawColumns.DIA: r.get("recebimento", "2026-01-02"),
...
def test_insert_conta_novos_e_ignora_existentes():
    ...
    assert inseridos == 2
    assert len(client.rows(DB_TABLE_RECEBIMENTO)) == 2
