...
from app_envios.core.config import Columns, RawColumns, SUPABASE_TABLE_ENVIOS
...
def test_insert_conta_novos_e_ignora_existentes():
    client = FakeSupabaseClient({SUPABASE_TABLE_ENVIOS: []})
    df = _raw_df([{"ordem": "1", "oficina": "A"}, {"ordem": "2", "oficina": "B"}])

    inseridos = insert_bulk_records(df, client=client)
    assert inseridos == 2
    assert len(client.rows(SUPABASE_TABLE_ENVIOS)) == 2
