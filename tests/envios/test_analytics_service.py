"""Testes de app_envios.services.analytics_service."""
...
from app_envios.services.data_cleaning import clean_dataframe


def _df() -> pd.DataFrame:
    raw = pd.DataFrame({
        RawColumns.ORIGEM: ["JEANS"] * 4,
        ...
        RawColumns.ENVIO: ["2026-01-05", "2026-02-10", "2026-01-06", None],
        RawColumns.MP: ["JEANS", "MALHA", "JEANS", "JEANS"],
        RawColumns.PDV: ["NAO_PDV"] * 4,
        RawColumns.FRETE: ["R.A"] * 4,
        RawColumns.SITUACAO: ["Enviado"] * 4,
    })
...
def test_compute_kpis_vazio():
    ...
    assert k == {"pecas": 0, "minutos": 0.0, "envios": 0, "oficinas": 0, "ordens": 0}
