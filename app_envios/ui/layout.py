"""
ui/layout.py — orquestração da página de Envios.
...
from app_envios.core.errors import guard
from app_envios.core.utils import format_int_br, format_minutos_br
from app_envios.services.analytics_service import (
    aggregate_by,
    compute_kpis,
    series_por_mes,
    series_por_semana,
...
from app_envios.ui.components.cards import KpiCardData, render_kpi_cards
from app_envios.ui.components.charts import build_periodo_chart, build_top_oficinas_chart


@guard("renderizar os indicadores de envios")
def render_kpi_section(df_filtered: pd.DataFrame) -> None:
    """Cards com os totais do recorte filtrado: peças, minutos, envios e oficinas."""
    kpis = compute_kpis(df_filtered)
    cards = [
        KpiCardData("Peças Enviadas", format_int_br(kpis["pecas"]), f'{format_int_br(kpis["ordens"])} ordens'),
        KpiCardData("Minutos Enviados", format_minutos_br(kpis["minutos"])),
        KpiCardData("Nº de Envios", format_int_br(kpis["envios"]), "linhas de envio"),
    ]
    render_kpi_cards(cards)


def _render_metric_table(agg: pd.DataFrame, dim_label: str) -> None:
    """Tabela HTML (rótulo · peças · minutos · envios) para a granularidade escolhida."""
    if agg.empty:
    ...
    for _, row in agg.iterrows():
        linhas.append(
            ...
            f'<td class="num">{format_int_br(row["envios"])}</td>'
            "</tr>"
        )
    html = (
        ...
        '<th class="num">Peças</th><th class="num">Minutos</th><th class="num">Envios</th>'
        "</tr></thead>"
...
@guard("renderizar a granularidade de envios")
...
@guard("renderizar os gráficos de envios")
def render_charts_section(df_filtered: pd.DataFrame, mes_selecionado: str | None = None) -> None:
    """
    Gráfico das 10 oficinas que mais receberam peças + UM gráfico de envios por
    período que se adapta ao filtro de mês:
      • Sem mês selecionado ("Todos os meses") → peças por MÊS.
      • Com um mês selecionado → peças por SEMANA daquele mês.
    """
    st.markdown("#### Top oficinas que mais receberam peças")
...
def _format_envio(value) -> str:
    if value is None or pd.isna(value):
        return "Sem data"
    return pd.Timestamp(value).strftime("%d/%m/%Y")


@guard("renderizar a tabela de consulta de envios")
def render_consulta_table(df_filtered: pd.DataFrame) -> None:
    """
    Tabela de consulta por número da ordem, com as colunas
    [Ordem, Oficina, Qtd, Minutos, Envio, MP], busca por ORDEM e paginação
    de 15 linhas por página.
    """
    st.markdown("#### Consulta de envios por ordem")
    st.caption("Busque pelo número da ordem ou navegue pelas páginas (15 por página).")

    busca = st.text_input(
        "Número da ordem",
        key="envios_busca_ordem",
        placeholder="Digite o número da ordem (ex.: 300222101)",
    ).strip()
    ...
    df = df.sort_values([Columns.ENVIO, Columns.ORDEM], na_position="last")
    total = len(df)

    if total == 0:
        st.info("Nenhum envio encontrado para a consulta atual.")
        return

    total_paginas = max(1, math.ceil(total / PAGE_SIZE_TABLE))

    # Estado de paginação. A busca reinicia para a página 1.
    if st.session_state.get("_envios_busca_anterior") != busca:
        st.session_state.envios_pagina = 1
        st.session_state._envios_busca_anterior = busca
    pagina = st.session_state.get("envios_pagina", 1)
    pagina = min(max(1, pagina), total_paginas)

    col_prev, col_info, col_next = st.columns([1, 3, 1], vertical_alignment="center")
    with col_prev:
        if st.button("← Anterior", use_container_width=True, disabled=pagina <= 1, key="envios_prev"):
            st.session_state.envios_pagina = pagina - 1
            st.rerun()
    with col_next:
        if st.button("Próxima →", use_container_width=True, disabled=pagina >= total_paginas, key="envios_next"):
            st.session_state.envios_pagina = pagina + 1
            st.rerun()
    with col_info:
        ...
        st.markdown(
            f'<div class="envios-pager"><span class="pg-info">Mostrando <strong>{ini}</strong>–'
    ...
    for _, row in pagina_df.iterrows():
        linhas.append(
            ...
            f'<td class="num">{format_int_br(row[Columns.QTD])}</td>'
            f'<td class="num">{format_minutos_br(row[Columns.MINUTOS])}</td>'
            f'<td style="text-align:center;">{_format_envio(row[Columns.ENVIO])}</td>'
