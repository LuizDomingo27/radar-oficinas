"""
ui/styles.py — CSS da área "Envios".

Reaproveita o CSS do DESIGN SYSTEM compartilhado (mesma paleta do Radar —
decisão "manter a cor do Radar para ambos") já definido em
``app_postos.ui.styles`` e acrescenta apenas o que é específico de Envios
(barra de paginação e cabeçalho de dimensão). Manter uma única fonte de verdade
para o tema evita divergência visual entre as áreas do app unificado.
"""
...
.envios-pager .pg-info {
    font-family: var(--font-body);
}
.envios-pager .pg-info strong {
...
def build_css() -> str:
    """CSS compartilhado (design system) + acréscimos de Envios num ÚNICO <style>.
    ...
    """
    return _build_shared_css().replace("</style>", _ENVIOS_EXTRA_RULES + "\n</style>")
