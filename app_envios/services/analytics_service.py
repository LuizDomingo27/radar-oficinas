"""
services/analytics_service.py — agregações da área "Envios".
...
oficinas que mais receberam peças e as séries por mês/semana dos gráficos.
...
# Dimensões temporais: linhas sem data de envio não têm como ser posicionadas.
...
def compute_kpis(df: pd.DataFrame) -> dict:
    ...
    if df.empty:
        return {"pecas": 0, "minutos": 0.0, "envios": 0, "oficinas": 0, "ordens": 0}
    return {
...
def aggregate_by(df: pd.DataFrame, granularity_key: str) -> pd.DataFrame:
    ...
    Soma peças e minutos por dimensão. Retorna colunas:
      rotulo · qtd · minutos · envios

    Para dimensões temporais (mês/semana/dia), linhas sem data de envio são
    excluídas (não há período onde colocá-las); para MP/Oficina, tudo entra.
    """
    ...
    if granularity_key in _TEMPORAL:
        base = df[df[Columns.ENVIO].notna()]

    if base.empty:
        return pd.DataFrame(columns=["rotulo", Columns.QTD, Columns.MINUTOS, "envios"])
    ...
    out = pd.DataFrame({
        ...
        "envios": grp["envios"].astype("int64"),
    })
...
def top_oficinas_por_pecas(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    """As `top_n` oficinas que mais receberam peças. Colunas: rotulo · qtd · minutos."""
    if df.empty:
...
def series_por_mes(df: pd.DataFrame) -> pd.DataFrame:
    """Série de peças enviadas por mês (ordenada). Colunas: rotulo · qtd · minutos."""
    return aggregate_by(df, "mes")[["rotulo", Columns.QTD, Columns.MINUTOS]]


def series_por_semana(df: pd.DataFrame) -> pd.DataFrame:
    """Série de peças enviadas por semana ISO (ordenada). Colunas: rotulo · qtd · minutos.
