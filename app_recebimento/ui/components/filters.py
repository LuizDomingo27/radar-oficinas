"""
ui/components/filters.py — filtros no topo da página de Recebimento.

Cascata Ano → Mês → MP → Oficina. O Ano é obrigatório (inicia no mais recente),
pois toda a análise é "referente ao ano filtrado". Linhas sem data de recebimento
não pertencem a nenhum ano e ficam de fora do recorte anual (informado ao usuário).
"""
...
def render_filters(df: pd.DataFrame) -> tuple[pd.DataFrame, int, str | None]:
    ...
    `qtd_sem_data_excluidas` = linhas sem data de recebimento descartadas pelo
    filtro de ano (para exibir um aviso transparente na UI).
    ...
    with col_ano:
        ano_sel = st.selectbox("Ano de Análise", options=anos, index=0, key="receb_filtro_ano")

    sem_data = int(df[Columns.RECEBIMENTO].isna().sum())
