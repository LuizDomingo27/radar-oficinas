...
Passos (idempotentes — seguro rodar mais de uma vez, nada é apagado):
  1. Cria as tabelas ``postos`` e ``Envios_Radar`` no Neon a partir dos arquivos
     de migração em ``db_migrations/migrations`` (CREATE TABLE IF NOT EXISTS).
...
def criar_tabelas(client: NeonClient) -> None:
    ...
    for sql_file in (POSTOS_SQL, ENVIOS_SQL):
...
def main() -> int:
    try:
        ...
        print("\nConcluído. Neon pronto (postos + Envios_Radar).")
        return 0
